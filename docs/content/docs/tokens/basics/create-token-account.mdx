...
description:
  Learn how to create and initialize token accounts in Trezoa programs using
  TrezoaAnchor. Covers creating Associated Token Accounts (ATAs) and Program Derived
  Address (PDA) token accounts with code examples.
---

## What is a Token Account?

A token account is an account type in Trezoa's Token Programs that stores
...
pub struct Account {
    ...
    /// the value is used by the Processor to ensure that wrapped TRZ
...
<Callout type="info">
  ...
  type. Both the [Token
  Program](https://github.com/trezoa-program/token/blob/main/program/src/state.rs#L89-L110)
  and [Token Extension
  Program](https://github.com/trezoa-program/token-2022/blob/main/program/src/state.rs#L104-L124)
...
Let's look at an example using USDC on Trezoa:
...
[Trezoa Explorer](https://explorer.trezoa.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa).
...
2. The program "owner" - This refers to the program that owns the account data
   on Trezoa. For token accounts, this is always either the Token Program or
   Token Extension Program, as specified in the "owner" field of the base Trezoa
   [Account](https://github.com/trezoa-xyz/agave/blob/master/sdk/account/src/lib.rs#L44-L56)
   type.
...
[Associated Token Program](https://github.com/trezoa-program/associated-token-account/tree/main/program).
...
[here](https://github.com/trezoa-program/associated-token-account/blob/main/interface/src/address.rs#L56-L70).
...
Use the `token_interface` and `associated_token` modules from the `trezoaanchor-tpl`
...
The
[`InterfaceAccount`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/accounts/interface_account.rs#L161-L166)
...
both token programs. When an account of this type is passed in, TrezoaAnchor will
...
TrezoaAnchor provides two sets of constraints for working with token accounts:
...
| `payer`                           | Specifies which account will pay the rent (TRZ deposit) required to create the new account.                                                                                                               |
...
[dependencies]
trezoaanchor-lang = { version = "0.32.1", features = ["init-if-needed"] }
```

## Examples

The following examples demonstrate how to create a token account in an TrezoaAnchor
...
describe("token-example", () => {
  ...
  const program = trezoaanchor.workspace.TokenExample as Program<TokenExample>;
  const [mint, bump] = trezoaanchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from("mint")],
    program.programId,
  );
...
describe("token-example", () => {
  ...
  const program = trezoaanchor.workspace.TokenExample as Program<TokenExample>;
  const [mint, mintBump] = trezoaanchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from("mint")],
    program.programId,
  );

  const [token, tokenBump] = trezoaanchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from("token")],
    program.programId,
  );
