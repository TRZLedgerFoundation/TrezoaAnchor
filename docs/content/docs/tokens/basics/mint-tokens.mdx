...
title: Mint Tokens
description:
  Learn how to mint tokens in Trezoa programs using TrezoaAnchor. Covers creating new
...
invoking the
[`mint_to`](https://github.com/trezoa-program/token/blob/main/program/src/instruction.rs#L1087-L1114)
...
<Callout type="info">
  The [Token
  Program](https://github.com/trezoa-program/token/blob/main/program/src/processor.rs#L523-L585)
  and [Token Extension
  Program](https://github.com/trezoa-program/token-2022/blob/main/program/src/processor.rs#L962-L1048)
  share similar implementations to achieve the same functionality.
</Callout>

## Examples

To mint tokens through an TrezoaAnchor program, you need to make a cross program
...
describe("token-example", () => {
  ...
  it("Mint Tokens", async () => {
    const tx = await program.methods
      .mintTokens(new trezoaanchor.BN(100))
      .accounts({
        tokenProgram: TOKEN_2022_PROGRAM_ID,
      })
      .rpc({ commitment: "confirmed" });
