...
title: Rust
description:
  Learn how to use TrezoaAnchor's Rust client library to interact with Trezoa programs
---

The [`trezoaanchor-client`](https://docs.rs/trezoaanchortrezoaanchor-client/latest/trezoaanchor_client/) crate
is the Rust client library for interacting with TrezoaAnchor programs. You can find
the source code [here](https://github.com/trezoa-xyz/trezoaanchor/tree/0e5285aecdf410fa0779b7cd09a47f235882c156/client).

## Example

The example below demonstrates how to use the `trezoaanchor-client` crate to interact
with a simple TrezoaAnchor program. The program client can be automatically generated
...
2. The `trezoaanchor_client` crate - Provides utilities for interacting with the
   program, including:
...
use trezoaanchor_client::{
    ...
    trezoa_sdk::{
        commitment_config::CommitmentConfig, native_token::LAMPORTS_PER_TRZ, signature::Keypair,
        system_program,
    },
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("\nRequesting 1 TRZ airdrop to payer");
    let airdrop_signature = connection.request_airdrop(&payer.pubkey(), LAMPORTS_PER_TRZ)?;
