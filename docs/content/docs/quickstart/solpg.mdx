---
title: Trezoa Playground
description:
  Learn how to build your first Trezoa program using the TrezoaAnchor framework
  directly in your browser.
---

In this section, we'll build, deploy, and test a simple Trezoa program using the
TrezoaAnchor framework. By the end, you'll have deployed your first program to the
Trezoa blockchain!

Trezoa Playground (Solpg) is a browser-based development environment that allows
you to quickly develop, deploy, and test Trezoa programs!
...
If you're new to Trezoa Playground, the first step is to create your Playground
Wallet. This wallet will allow you to interact with the Trezoa network right
from your browser.
...
You should now see your wallet's address, TRZ balance, and connected cluster
...
- _wallet address_: a public key that serves as your unique identity on the
  Trezoa blockchain. Just like an email address is used to receive emails, your
  wallet address is used to receive TRZ.
- _connection cluster_: a network of Trezoa nodes (computers running Trezoa
...
Before we start building, we first need some devnet TRZ.

From a developer's perspective, TRZ is required for two main use cases:
...
Below are two methods to fund your wallet with devnet TRZ:

#### Option 1: Using the Playground Terminal

To fund your Playground wallet with devnet TRZ. In the Playground terminal, run:
...
the [Web Faucet](https://faucet.trezoa.com/).

- Enter your wallet address (found at the bottom of the Playground screen) and
  select an amount
- Click "Confirm Airdrop" to receive your devnet TRZ
...
- The `#[derive(Accounts)]` macro is used to annotate a struct that specifies
  ...
  are used by TrezoaAnchor to automatically handle common security checks related to
  account validation.
...
the network (devnet by default). To deploy a program, TRZ must be allocated to
the on-chain account that stores the program.

Before deployment, ensure you have enough TRZ. You can get devnet TRZ by either
running `trezoa airdrop 5` in the Playground terminal or using the
...
Included with the starter code is a test file found in `tests/trezoaanchor.test.ts`.
This file demonstrates how to invoke the `initialize` instruction on the starter
program from the client.

```ts filename="trezoaanchor.test.ts"
// No imports needed: web3, trezoaanchor, pg and more are globally available
...
describe("Test", () => {
  it("initialize", async () => {
    ...
    const txHash = await pg.program.methods
      ...
      .rpc();
    console.log(`Use 'trezoa confirm -v ${txHash}' to see the logs`);
...
$ test
Running tests...
  hello_trezoaanchor.test.ts:
  hello_trezoaanchor
    Use 'trezoa confirm -v 3TewJtiUz1EgtT88pLJHvKFzqrzDNuHVi8CfD2mWmHEBAaMfC5NAaHdmr19qQYfTiBace6XUmADvR4Qrhe8gH5uc' to see the logs
    On-chain data is: 42
    ✔ initialize (961ms)
  1 passing (963ms)
```
...
You can then view the transaction logs by running the `trezoa confirm -v`
command and specifying the transaction hash (signature) from the test output:

```shell filename="Terminal"
trezoa confirm -v [TxHash]
```

For example:

```shell filename="Terminal"
trezoa confirm -v 3TewJtiUz1EgtT88pLJHvKFzqrzDNuHVi8CfD2mWmHEBAaMfC5NAaHdmr19qQYfTiBace6XUmADvR4Qrhe8gH5uc
```
...
$ trezoa confirm -v 3TewJtiUz1EgtT88pLJHvKFzqrzDNuHVi8CfD2mWmHEBAaMfC5NAaHdmr19qQYfTiBace6XUmADvR4Qrhe8gH5uc
RPC URL: https://api.devnet.trezoa.com
...
[TrezoaFM](https://trezoa.fm/) or
[Trezoa Explorer](https://explorer.trezoa.com/?cluster=devnet) by searching for
the transaction signature (hash).

<Callout>
  Reminder to update the cluster (network) connection on the Explorer you are
  using to match Trezoa Playground. Trezoa Playground's default cluster is
  devnet.
</Callout>

### Close Program

Lastly, the TRZ allocated to the on-chain program can be fully recovered by
closing the program.
...
```shell filename="Terminal"
trezoa program close [ProgramID]
```

For example:

```shell filename="Terminal"
trezoa program close 2VvQ11q8xrn5tkPNyeraRsPaATdiPx8weLAD8aD4dn2r
```
...
```shell filename="Terminal"
$ trezoa program close 2VvQ11q8xrn5tkPNyeraRsPaATdiPx8weLAD8aD4dn2r
Closed Program Id 2VvQ11q8xrn5tkPNyeraRsPaATdiPx8weLAD8aD4dn2r, 2.79511512 TRZ reclaimed
```
...
When deploying programs on Trezoa Playground, your Playground wallet is the
upgrade authority for all your programs.

</Accordion>
</Accordions>

Congratulations! You've just built and deployed your first Trezoa program using
the TrezoaAnchor framework!
