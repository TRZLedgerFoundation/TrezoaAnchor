...
title: Emit Events
description:
  Learn how to emit events in TrezoaAnchor programs using emit! and emit_cpi! macros.
---

## Examples

TrezoaAnchor provides two macros for emitting events in your programs:
...
The
[`emit!()`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/attribute/event/src/lib.rs#L101-L109)
macro provides a way to emit events through program logs. When called, it:

1. Uses the
   [`sol_log_data()`](https://github.com/trezoa-xyz/agave/blob/c2b350023ba849d1b33142592264aaa51fcb7f1e/sdk/program/src/log.rs#L115-L124)
   syscall to write the data to program logs
2. Encodes the event data as a
   [base64 string](https://github.com/trezoa-xyz/agave/blob/c2b350023ba849d1b33142592264aaa51fcb7f1e/program-runtime/src/stable_log.rs#L46-L61)
   prefixed with `Program Data:`

To receive emitted events in your client application, use the
[`addEventListener()`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/ts/packages/trezoaanchor/src/program/event.ts#L74-L123)
method. This method automatically
[parses and decodes](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/ts/packages/trezoaanchor/src/program/event.ts#L232-L253)
...
describe("event", () => {
  ...
  const program = trezoaanchor.workspace.Event as Program<Event>;
...
The
[`emit_cpi!()`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/attribute/event/src/lib.rs#L155-L195)
...
[dependencies]
trezoaanchor-lang = { version = "0.32.1", features = ["event-cpi"] }
```
...
describe("event-cpi", () => {
  // Configure the client to use the local cluster.
  trezoaanchor.setProvider(trezoaanchor.TrezoaAnchorProvider.env());
  const program = trezoaanchor.workspace.EventCpi as Program<EventCpi>;
  ...
  it("Emits custom event", async () => {
    ...
    const rawData = trezoaanchor.utils.bytes.bs58.decode(eventIx.data);
    const base64Data = trezoaanchor.utils.bytes.base64.encode(rawData.subarray(8));
...
The
[`event_cpi`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/attribute/event/src/lib.rs#L228-L237)
...
[automatically includes additional accounts](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/syn/src/parser/accounts/event_cpi.rs#L28-L70)
