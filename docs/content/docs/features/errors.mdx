---
title: Custom Errors
description: Learn how to implement custom error handling in TrezoaAnchor programs.
---

All instruction handlers in TrezoaAnchor programs return a custom `Result<T>` type
...
The
[`Result<T>`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/lib.rs#L77)
type in TrezoaAnchor programs is a type alias that wraps the standard Rust
`Result<T, E>`. In this case, `T` represents the successful return type, while
`E` is TrezoaAnchor's custom `Error` type.
...
When an error occurs in an TrezoaAnchor program, it returns a custom
[`Error`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/error.rs#L277-L281)
type defined as:
...
The `Error` type in TrezoaAnchor programs can be one of two variants:

1. [`ProgramErrorWithOrigin`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/error.rs#L389-L394):
   Custom type that wraps a standard Trezoa
   [`ProgramError`](https://github.com/trezoa-xyz/agave/blob/v1.18.26/sdk/program/src/program_error.rs#L12-L66)
   type. These errors come from the `trezoa_program` crate.
...
2. [`TrezoaAnchorError`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/error.rs#L490-L497):
   Errors defined by the TrezoaAnchor framework.
...
An `TrezoaAnchorError` can be thought of as having two categories:

1. Internal TrezoaAnchor Errors - These are built-in errors included with the TrezoaAnchor
   framework. They are defined in the
   [`ErrorCode`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/error.rs#L10-L275)
   enum.

2. Custom Program Errors - These are program specific errors that developers
   define to handle custom error cases.

The `error_code_number` from an `TrezoaAnchorError` has the following numbering
scheme:
...
TrezoaAnchor provides a convenient way to define custom errors through the
`error_code` attribute. The implementation details can be found
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/master/lang/syn/src/codegen/error.rs).

When you define an enum with the `error_code` attribute, TrezoaAnchor automatically:
...
[`err!`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/lib.rs#L735-L743)
...
`TrezoaAnchorError`. The implementation can be found
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/attribute/error/src/lib.rs#L84-L116).
...
The
[`require!`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/lib.rs#L525-L537)
...
TrezoaAnchor provides several "require" macros for different validation needs. You can
find the implementation of these macros
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/lib.rs).
...
an TrezoaAnchor program. The program below validates that an input amount falls within
...
describe("custom-error", () = {
  ...
  const program = trezoaanchor.workspace.CustomError as Program<CustomError;

  it("Successfully validates amount within range", async () = {
    const tx = await program.methods.validateAmount(new trezoaanchor.BN(50)).rpc();
  ...
  it("Fails with amount too small", async () => {
    try {
      await program.methods.validateAmount(new trezoaanchor.BN(5)).rpc();
  ...
  it("Fails with amount too large", async () => {
    try {
      await program.methods.validateAmount(new trezoaanchor.BN(150)).rpc();
...
When a program error occurs, TrezoaAnchor's TypeScript Client SDK returns a detailed
[error response](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/ts/packages/trezoaanchor/src/error.ts#L51-L71)
...
{
  errorLogs: [
    'Program log: TrezoaAnchorError thrown in programs/custom-error/src/lib.rs:11. Error Code: AmountTooLarge. Error Number: 6001. Error Message: Amount must be less than or equal to 100.'
  ],
  logs: [
...
[errors test program](https://github.com/trezoa-xyz/trezoaanchor/blob/master/tests/errors/programs/errors/src/lib.rs)
in the TrezoaAnchor repository.
