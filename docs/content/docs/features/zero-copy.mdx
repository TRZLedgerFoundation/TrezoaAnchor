...
title: Zero Copy
description:
  Learn how to use TrezoaAnchor's zero-copy deserialization feature to handle large
  account data in Trezoa programs.
---
...
for handling large accounts efficiently on Trezoa.
...
[`#[account(zero_copy)]`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/attribute/account/src/lib.rs#L417).
...
[`AccountLoader<'info, T>`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/accounts/account_loader.rs#L95-L99),
...
[`load_init`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/accounts/account_loader.rs#L199-L221)
...
[`zero`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/syn/src/codegen/accounts/constraints.rs#L200-L264)
...
create accounts up to Trezoa's maximum account size of 10MB (10_485_760 bytes),
...
Use
[`load_mut`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/accounts/account_loader.rs#L172-L195)
...
Use
[`load`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/accounts/account_loader.rs#L154-L169)
...
Zero-copy types cannot derive `TrezoaAnchorSerialize`/`TrezoaAnchorDeserialize`. Use
...
// [!code word:TrezoaAnchorSerialize]
...
describe("zero-copy", () => {
  ...
  const program = trezoaanchor.workspace.ZeroCopy as Program<ZeroCopy>;
  const dataAccount = trezoaanchor.web3.Keypair.generate();
...
Note that the maximum Trezoa account size is 10MB (10_485_760 bytes), 8 bytes
...
describe("zero-copy-two", () => {
  ...
  const program = trezoaanchor.workspace.ZeroCopyTwo as Program<ZeroCopyTwo>;
  const dataAccount = trezoaanchor.web3.Keypair.generate();
  ...
  it("Is initialized!", async () => {
    ...
    // [!code highlight:7]
    const createAccountInstruction = trezoaanchor.web3.SystemProgram.createAccount({
    ...
    const transaction = new trezoaanchor.web3.Transaction().add(
