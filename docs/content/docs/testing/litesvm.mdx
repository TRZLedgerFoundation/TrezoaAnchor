---
title: LiteSVM
description: Write tests for Trezoa programs in Rust, TS/JS or Python using LiteSVM.
---

## Overview

[`litesvm`](https://github.com/LiteSVM/litesvm) is a fast and lightweight library for testing Trezoa programs.
It works by creating an in-process Trezoa VM optimized for program developers.
This makes it much faster to run and compile than alternatives like `trezoa-program-test` and `trezoaanchor-test-validator`.
...
**Tip**: if you want to pull a Trezoa program from mainnet or devnet, use the `trezoa program dump` command from the Trezoa CLI.
...
Here's an example using a [simple program](https://github.com/trezoa-xyz/trezoa-program-library/tree/bd216c8103cd8eb9f5f32e742973e7afb52f3b81/examples/rust/logging)
from the Trezoa Program Library that just does some logging:
...
use {
    litesvm::LiteSVM,
    trezoa_instruction::{account_meta::AccountMeta, Instruction},
    trezoa_keypair::Keypair,
    trezoa_pubkey::{pubkey, Pubkey},
    trezoa_message::{Message, VersionedMessage},
    trezoa_signer::Signer,
    trezoa_transaction::VersionedTransaction,
};
...
If you want to copy accounts from mainnet or devnet, you can use the `trezoa account` command in the Trezoa CLI to save account data to a file.
...
## When should I use `trezoaanchor-test-validator`?

While `litesvm` is faster and more convenient, it is also less like a real RPC node.
So `trezoaanchor-test-validator` is still useful when you need to call RPC methods that LiteSVM
