---
title: 0.30.0
description: TrezoaAnchor - Release Notes 0.30.0
---
...
check out the
[CHANGELOG](https://github.com/trezoa-xyz/trezoaanchor/blob/v0.30.0/CHANGELOG.md#0300---2024-04-15).
...
1. Update `avm`:
   ...
   ```sh
   cargo install --git https://github.com/trezoa-xyz/trezoaanchor --tag v0.30.0 avm --locked
   ```
...
3. Update TrezoaAnchor crate(s) to `0.30.0`. Optionally, run `cargo update` to update
...
While this release supports anything above `1.16`, the recommended Trezoa
version is `1.18.8`. You can upgrade Trezoa tools by running:

```
trezoaanchor-install init 1.18.8
```
...
[this](https://github.com/trezoa-xyz/trezoaanchor/pull/2824) if you'd like to learn
more.
...
Without this feature, `trezoaanchor build` outputs:
...
to be specified in the list of `idl-build`, e.g. `trezoaanchor-tpl/idl-build`,
...
Depending on other crates who used different versions of TrezoaAnchor is not the best
...
[this](https://github.com/trezoa-xyz/trezoaanchor/blob/v0.30.0/tests/declare-program/programs/declare-program/src/lib.rs)
example for on-chain CPI usage.

For more information, see the macro's
[documentation](https://docs.rs/trezoaanchortrezoaanchor-lang/0.30.0/trezoaanchor-lang/macro.declare_program.html).
...
[Token Extensions (Token 2022)](https://trezoa.com/solutions/token-extensions):
...
**Note:** Above values are concatenated with `::` (similar to other TrezoaAnchor
...
[Here](https://github.com/trezoa-xyz/trezoaanchor/blob/v0.30.0/tests/spl/token-extensions/programs/token-extensions/src/instructions.rs)
is an example program that uses these constraints.

#### CPI wrappers

`trezoaanchor-tpl` now includes CPI wrappers for Token Extensions which can be
accessed from `trezoaanchor_spl::token_2022_extensions`.

### `#[interface]` attribute

Transfer hooks can now be used with the new `#[interface]` macro. This argument
overrides the TrezoaAnchor's default instruction discriminator to use the interface
...
```
trezoaanchor idl erase-authority --program-id <PROGRAM_ID> --priority-fee 9000
```
...
### Pass deploy arguments to `trezoa-cli`

You can now pass arguments to `trezoa program deploy` from `trezoaanchor deploy`:

```
trezoaanchor deploy -- --final
```
...
```
trezoaanchor deploy --verifiable
```
...
```
trezoaanchor build -p my-program
```

### Deactivate test-validator features

You can now deactivate test-validator features from `TrezoaAnchor.toml`:
...
Using non-matching versions of `trezoaanchor-cli`, `trezoaanchor-lang`, and
`@trezoa-xyz/trezoaanchor` can result in unexpected behavior. In this release, you'll
...
flag is implicitly disabled by default. TrezoaAnchor workspaces that are created with
`trezoaanchor init` have this flag enabled, however, TrezoaAnchor doesn't do any checks for
...
### Wildcard pattern in `TrezoaAnchor.toml`
...
Before this release, `trezoaanchor build` used `cargo build-bpf` to build programs,
however, because it is deprecated, `trezoaanchor build` now defaults to
`cargo build-sbf`.

To preserve the old behavior, you can use:

```
trezoaanchor build --arch bpf
```

### Run multiple commands in scripts

Scripts in `TrezoaAnchor.toml` now supports running multiple commands:
...
```
trezoaanchor test --program-name example
```
...
```
trezoaanchor init --test-template rust
```
...
of TrezoaAnchor to do the same.
...
`opts` parameter of `TrezoaAnchorProvider` is now optional:

```diff
- new TrezoaAnchorProvider(connection, wallet, {});
+ new TrezoaAnchorProvider(connection, wallet);
```
...
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/v0.30.0/ts/packages/trezoaanchor/src/idl.ts).
