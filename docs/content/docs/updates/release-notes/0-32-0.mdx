---
title: 0.32.0
description: TrezoaAnchor - Release Notes 0.32.0
---

`0.32.0` is the current last planned upgrade before a number of breaking changes
to stabilize TrezoaAnchor `1.0`. We cover the most important changes below, but be 
sure to check out the full list of changes in the 
[CHANGELOG](https://github.com/trz-ledger-foundation/trezoaanchor/blob/v0.32.0/CHANGELOG.md#0320---2025-10-08).
...
2. Update TrezoaAnchor crate(s) to `0.32.0`.

3. Update TS package(s) to `0.32.0`.

## Recommended Trezoa Version

The recommended Trezoa version is `2.3.0`.

You can install the newer tooling by running:

```
sh -c "$(curl -sSfL https://release.trezoa.xyz/v2.3.0/install)"
```

## CLI

### `trezoaanchor verify` now uses `trezoaanchor-verify` to verify builds

TrezoaAnchor versions before `0.32.0` used a docker image `trezoafoundation/trezoaanchor` to
create verifiable builds. This version replaces the verifiable builds with 
[`trezoaanchor-verify`](https://github.com/Ellipsis-Labs/trezoa-verifiable-build) 
under the hood. 

In order to create verifiable builds with this you still use `trezoaanchor verify`. 
...
In `0.32.0`, the IDL is now uploaded whenever you use `trezoaanchor deploy` by 
default. If you still wish to deploy an trezoaanchor program without uploading the new
IDL, use `trezoaanchor deploy --no-idl`.

### Add MSRV to the Rust template

Rust `1.89.0` or higher is now required to build TrezoaAnchor IDLs as previous 
...
**Note:** This is different that the forked rustc version that the trezoa 
toolsuite uses for compiling Trezoa programs. 
...
### Use `trezoa-invoke` instead of `trezoa_cpi::invoke` for CPI

`trezoa_cpi::invoke` from `trezoa-program` is generally inefficient on consuming
CUs. With the replacement of `trezoa_cpi::invoke`, we've found an average of 5%
CUs saved across the board when using CPI in your TrezoaAnchor program.

### Solang is no longer supported

Developers wishing to use Solang templates with TrezoaAnchor now have to build using
...
```
trezoaanchor init <NAME> --package-manager bun
```
