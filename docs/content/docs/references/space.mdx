...
`space` constraint for TrezoaAnchor's internal discriminator (see the example).
...
#[derive(TrezoaAnchorSerialize, TrezoaAnchorDeserialize, Clone, PartialEq, Eq)]
pub enum GameState {
    Active,
    Tie,
    Won { winner: Pubkey },
...
pub struct InitializeMyData<'info> {
    // Note that we have to add 8 to the space for the internal trezoaanchor
    // [!code highlight]
