...
description:
  Learn about the Interface Description Language (IDL) file in TrezoaAnchor, its
  purpose, benefits, and how it simplifies program-client interactions
---

An Interface Description Language (IDL) file for an TrezoaAnchor program provides a
...
<Callout type="info">
The `trezoaanchor build` command generates an IDL file located at
...
import { Program, BN } from "@trezoa-xyz/trezoaanchor";
import { HelloTrezoaAnchor } from "../target/types/hello_trezoaanchor";
import { Keypair } from "@trezoa/web3.js";
import assert from "assert";

describe("hello_trezoaanchor", () => {
  const provider = trezoaanchor.TrezoaAnchorProvider.env();
  trezoaanchor.setProvider(provider);
  const wallet = provider.wallet as trezoaanchor.Wallet;
  const program = trezoaanchor.workspace.HelloTrezoaAnchor as Program<HelloTrezoaAnchor>;
...
TrezoaAnchor assigns a unique 8 byte discriminator to each instruction and account
...
prefix combined with the instruction or account name. As of TrezoaAnchor v0.30, these
discriminators are included in the IDL file.

<Callout type="info">
  Note that when working with TrezoaAnchor, you typically won't need to interact
...
When an TrezoaAnchor program instruction is invoked, the discriminator is included as
...
You can find the implementation of the discriminator generation in the TrezoaAnchor
codebase
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/syn/src/codegen/program/common.rs#L5-L19), 
for the [`gen_discriminator` method here](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/syn/src/codegen/program/common.rs#L21-L24),
which is used
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/syn/src/codegen/program/instruction.rs#L33).
...
You can find the implementation of the discriminator generation in the TrezoaAnchor
codebase
[here](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/attribute/account/src/lib.rs#L101-L117).

<Callout type="info">
  Note that different programs using identical account names will generate the
  same discriminator. When deserializing account data, TrezoaAnchor programs will also
