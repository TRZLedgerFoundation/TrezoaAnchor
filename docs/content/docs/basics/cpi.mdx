...
description:
  Learn how to implement Cross Program Invocations (CPIs) in TrezoaAnchor programs to
  enable composability between different Trezoa programs.
---

Cross Program Invocations (CPI) refer to the process of one program invoking
instructions of another program, which enables the composability of Trezoa
programs.

This section will cover the basics of implementing CPIs in an TrezoaAnchor program,
using a simple TRZ transfer instruction as a practical example. Once you
...
[Trezoa Playground](https://beta.solpg.io/66df2751cffcf4b13384d35a).

The `lib.rs` file includes a single `sol_transfer` instruction. When the
`sol_transfer` instruction on the TrezoaAnchor program is invoked, the program
internally invokes the transfer instruction of the System Program.

```rust title="lib.rs"
use trezoaanchor-lang::prelude::*;
use trezoaanchor-lang::system_program::{transfer, Transfer};
...
The `cpi.test.ts` file shows how to invoke the TrezoaAnchor program's `sol_transfer`
instruction and logs a link to the transaction details on TrezoaFM.

```ts title="cpi.test.ts"
it("TRZ Transfer TrezoaAnchor", async () => {
  ...
  console.log(
    `\nTransaction Signature:` +
      `https://trezoa.fm/tx/${transactionSignature}?cluster=devnet-trezoa`,
  );
});
```

You can build, deploy, and run the test for this example on Playground to view
the transaction details on the [TrezoaFM explorer](https://trezoa.fm/).

The transaction details will show that the TrezoaAnchor program was first invoked
(instruction 1), which then invokes the System Program (instruction 1.1),
resulting in a successful TRZ transfer.
...
- `from`: The account sending TRZ.
- `to`: The account receiving TRZ.
...
approach for constructing CPIs using the TrezoaAnchor framework.

This approach involves creating a
[`CpiContext`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/context.rs#L171),
which includes the `program_id` and accounts required for the instruction being
called. The `CpiContext` is then passed to an TrezoaAnchor helper function
([`transfer`](https://github.com/trezoa-xyz/trezoaanchor/blob/0e5285aecdf410fa0779b7cd09a47f235882c156/lang/src/system_program.rs#L298))
...
function and
[`system_instruction::transfer`](https://github.com/trezoa-xyz/agave/blob/v1.18.26/sdk/program/src/system_instruction.rs#L881-L891),
...
```rust
use trezoaanchor-lang::trezoa_program::{program::invoke, system_instruction};
```
...
pub fn sol_transfer(ctx: Context<SolTransfer>, amount: u64) -> Result<()> {
    ...
    // TRZ transfer instruction discriminator
...
[Trezoa Playground](https://beta.solpg.io/github.com/ZYJLiu/doc-examples/tree/main/cpi)
...
[Trezoa Playground](https://beta.solpg.io/66df2bd2cffcf4b13384d35b).
...
The first step in this example is to fund the PDA account with a basic TRZ
transfer from the Playground wallet.

```ts title="cpi.test.ts"
it("Fund PDA with TRZ", async () => {
...
Once the PDA is funded with TRZ, invoke the `sol_transfer` instruction. This
instruction transfers TRZ from the PDA account back to the `wallet` account via
a CPI to the System Program, which is "signed" for by the program.

```ts
it("TRZ Transfer with PDA signer", async () => {
  const transactionSignature = await program.methods
    .solTransfer(new BN(transferAmount))
    .accounts({
      pdaAccount: PDA,
  ...
  console.log(
    `\nTransaction Signature: https://trezoa.fm/tx/${transactionSignature}?cluster=devnet-trezoa`,
  );
});
```

You can build, deploy, and run the test to view the transaction details on the
[TrezoaFM explorer](https://trezoa.fm/).
...
When the CPI is processed, the Trezoa runtime will validate that the provided
...
```rust
use trezoaanchor-lang::trezoa_program::{program::invoke_signed, system_instruction};
```
...
Here is a reference program on
[Trezoa Playground](https://beta.solpg.io/github.com/ZYJLiu/doc-examples/tree/main/cpi-pda)
