...
pub fn initialize_account<'info>(
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(
        &ix,
        &[
...
pub fn initialize_account3<'info>(
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(&ix, &[ctx.accounts.account, ctx.accounts.mint])
        .map_err(Into::into)
}
...
pub fn initialize_mint<'info>(
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(&ix, &[ctx.accounts.mint, ctx.accounts.rent])
        .map_err(Into::into)
}
...
pub fn initialize_mint2<'info>(
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(&ix, &[ctx.accounts.mint]).map_err(Into::into)
}
...
pub fn sync_native<'info>(ctx: CpiContext<'_, '_, '_, 'info, SyncNative<'info>>) -> Result<()> {
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(&ix, &[ctx.accounts.account]).map_err(Into::into)
}
...
pub fn get_account_data_size<'info>(
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(&ix, &[ctx.accounts.mint])?;
    trezoaanchor-lang::trezoa_program::program::get_return_data()
        .ok_or(trezoaanchor-lang::trezoa_program::program_error::ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != ctx.program_id {
                Err(trezoaanchor-lang::trezoa_program::program_error::ProgramError::IncorrectProgramId)
            } else {
                data.try_into().map(u64::from_le_bytes).map_err(|_| {
                    trezoaanchor-lang::trezoa_program::program_error::ProgramError::InvalidInstructionData
                })
            }
        })
        .map_err(Into::into)
...
pub fn amount_to_ui_amount<'info>(
    ...
    trezoaanchor-lang::trezoa_program::program::invoke(&ix, &[ctx.accounts.account])?;
    trezoaanchor-lang::trezoa_program::program::get_return_data()
        .ok_or(trezoaanchor-lang::trezoa_program::program_error::ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != ctx.program_id {
...
impl trezoaanchor-lang::Id for Token2022 {
    fn id() -> Pubkey {
        ID
    }
}
