...
## Reporting security problems for TrezoaAnchor

**DO NOT CREATE A GITHUB ISSUE** to report a security problem.

Instead please email trezoaanchor-security@trezoa.org. Provide a helpful title, detailed description of the vulnerability and an exploit proof-of-concept. Speculative submissions without proof-of-concept will be closed with no further consideration.
...
2. Was the bug public knowledge previously? This may mean that it's a vulnerability class for users of TrezoaAnchor, but not an issue within TrezoaAnchor itself
...
If you do not receive a response in the advisory, send an email to trezoaanchor-security@trezoa.org with the full URL of the advisory you have created. DO NOT include attachments or provide detail sufficient for exploitation regarding the security issue in this email. **Only provide such details in the advisory**.
...
In response a newly reported security problem, a member of the `trz-ledger-foundation/admins` group will accept the report to turn it into a draft advisory. The `trz-ledger-foundation/trezoaanchor-security-incident-response` group should be added to the draft security advisory, and create a private fork of the repository (grey button towards the bottom of the page) if necessary.
...
If the report is out of scope, a member of the `trz-ledger-foundation/admins` group will comment as such and then close the report.

### 2. Triage
Within the draft security advisory, discuss and determine the severity of the issue. If necessary, members of the `trz-ledger-foundation/trezoaanchor-security-incident-response` group may add other github users to the advisory to assist. If it is determined that this is not a critical TrezoaAnchor issue then the advisory should be closed and if more follow-up is required a normal TrezoaAnchor public github issue should be created.
...
Once an ETA is available for the fix, a member of the `trz-ledger-foundation/trezoaanchor-security-incident-response` group should notify major affected parties. The teams are all over the world and it's critical to provide actionable information at the right time. Don't be the person that wakes everybody up at 2am when a fix won't be available for hours.
...
At its sole discretion, the TRZ Ledger Foundation may offer a bounty for valid reports of TrezoaAnchor vulnerabilities. Please see below for more details. The submitter is not required to provide a mitigation to qualify.

#### IMPORTANT | PLEASE NOTE
_Note: Payments will continue to be paid out in 12-month locked TRZ._

#### Critical:
_Max: $100k in TRZ tokens. Min: $10k in TRZ tokens_

* Bypassing fundamental TrezoaAnchor checks, such as account ownership, discriminator, memory safety, etc.

#### Medium:
_Max: $25k in TRZ tokens. Min: $5k in TRZ tokens_

* Denial of service attacks

#### Low:
_Max: $5k in TRZ tokens. Min: $100 in TRZ tokens_
...
### Eligibility:
* Anyone under a grant or the financial arrangement with TRZ Ledger Foundation to develop or audit related tools is not eligible
...
* Participants must complete KYC and sign the participation agreement here when the registrations are open https://trezoa.org/kyc. Security exploits will still be assessed and open for submission at all times. This needs only be done prior to distribution of tokens.
