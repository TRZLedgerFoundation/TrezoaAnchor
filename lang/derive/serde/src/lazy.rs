...
pub fn gen_lazy(input: proc_macro::TokenStream) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let (name, generics, size, sized) = match &item {
        Item::Struct(strct) => (
            ...
            strct
                ...
                .iter()
                .map(|field| &field.ty)
                .map(|ty| quote! { <#ty as trezoaanchor-lang::__private::Lazy>::SIZED })
    ...
    Ok(quote! {
        impl #impl_generics trezoaanchor-lang::__private::Lazy for #name #ty_generics #where_clause {
...
fn sum_fields(fields: &Fields) -> proc_macro2::TokenStream {
    ...
    let declarations = fields.iter().enumerate().map(|(i, field)| {
        ...
        quote! { let #name = <#ty as trezoaanchor-lang::__private::Lazy>::size_of(#buf) }
    });
