...
pub fn derive_init_space(item: TokenStream) -> TokenStream {
    ...
    let process_struct_fields = |fields: Punctuated<Field, Comma>| {
        ...
        quote! {
            ...
            impl #impl_generics trezoaanchor-lang::Space for #name #ty_generics #where_clause {
    ...
    let expanded: TokenStream2 = match input.data {
        ...
        syn::Data::Enum(enm) => {
            ...
            quote! {
                #[automatically_derived]
                impl trezoaanchor-lang::Space for #name {
...
fn gen_max<T: Iterator<Item = TokenStream2>>(mut iter: T) -> TokenStream2 {
    if let Some(item) = iter.next() {
        ...
        quote!(trezoaanchor-lang::__private::max(#item, #next_item))
    } else {
        quote!(0)
    }
}
    ...
    match ty {
        ...
        Type::Path(ty_path) => {
            ...
            match type_name.as_str() {
                ...
                _ => {
                    let ty = &ty_path.path;
                    quote!(<#ty as trezoaanchor-lang::Space>::INIT_SPACE)
                }
            }
        }
        Type::Tuple(ty_tuple) => {
