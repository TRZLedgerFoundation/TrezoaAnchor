...
pub fn gen_lazy(strct: &syn::ItemStruct) -> syn::Result<TokenStream> {
    ...
    let lazy_acc_ty = quote! { trezoaanchor-lang::accounts::lazy_account::LazyAccount };
    let disc_len = quote! { <#ident as trezoaanchor-lang::Discriminator>::DISCRIMINATOR.len() };
    ...
    let (loader_signatures, loader_impls) = strct
        ...
        .map(|(i, field)| {
            ...
            let ty = &field.ty;
            let ty_as_lazy = quote! { <#ty as trezoaanchor-lang::__private::Lazy> };
            let size = quote! {
            ...
            let signatures = quote! {
                ...
                ///
                #load_panic_docs
                fn #load_ident(&self) -> trezoaanchor-lang::Result<::core::cell::Ref<'_, #ty>>;
                ...
                ///
                #load_mut_panic_docs
                fn #load_mut_ident(&self) -> trezoaanchor-lang::Result<::core::cell::RefMut<'_, #ty>>;

                 #[doc(hidden)]
                fn #load_common_ident<R>(&self, f: impl FnOnce() -> R) -> trezoaanchor-lang::Result<R>;
            ...
            let impls = quote! {
                ...
                fn #load_common_ident<R>(&self, f: impl FnOnce() -> R) -> trezoaanchor-lang::Result<R> {
                    ...
                    let val = trezoaanchor-lang::TrezoaAnchorDeserialize::try_from_slice(
    ...
    Ok(quote! {
        pub trait #lazy_ident {
            ...
            ///
            #load_panic_docs
            fn load(&self) -> trezoaanchor-lang::Result<::core::cell::Ref<'_, #ident>>;
            ...
            ///
            #load_mut_panic_docs
            fn load_mut(&self) -> trezoaanchor-lang::Result<::core::cell::RefMut<'_, #ident>>;
            ...
            fn exit(&self, program_id: &trezoaanchor-lang::prelude::Pubkey) -> trezoaanchor-lang::Result<()>;
        }
        ...
        impl<'info> #lazy_ident for #lazy_acc_ty<'info, #ident> {
            ...
            fn #load_common_ident<R>(&self, f: impl FnOnce() -> R) -> trezoaanchor-lang::Result<R> {
                ...
                if all_uninit {
                    ...
                    let val = trezoaanchor-lang::TrezoaAnchorDeserialize::deserialize(&mut &data[offset..])?;
            ...
            fn exit(&self, program_id: &trezoaanchor-lang::prelude::Pubkey) -> trezoaanchor-lang::Result<()> {
                ...
                if &<#ident as trezoaanchor-lang::Owner>::owner() == program_id
                    && !trezoaanchor-lang::__private::is_closed(self.__info)
                {
                    // Make sure all fields are initialized
                    let acc = self.load()?;
                    let mut data = self.__info.try_borrow_mut_data()?;
                    let dst: &mut [u8] = &mut data;
                    let mut writer = trezoaanchor-lang::__private::BpfWriter::new(dst);
