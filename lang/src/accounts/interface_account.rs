...
/// - [Using InterfaceAccount with non-trezoaanchor types](#using-interface-account-with-non-trezoaanchor-types)
...
/// ```
///
/// # Using InterfaceAccount with non-trezoaanchor programs
///
/// InterfaceAccount can also be used with non-trezoaanchor programs. The data types from
...
/// ### TPL Types
///
/// TrezoaAnchor provides wrapper types to access accounts owned by the token programs. Use
/// ```ignore
/// use trezoaanchor_spl::token_interface::TokenAccount;
///
/// #[derive(Accounts)]
/// pub struct Example {
...
/// ```ignore
/// use trezoaanchor_spl::token_interface::Mint;
///
/// #[derive(Accounts)]
/// pub struct Example {
...
impl<'a, T: AccountSerialize + AccountDeserialize + Clone> InterfaceAccount<'a, T> {
    ...
    ///
    /// No TrezoaAnchor discriminator is checked during reload. Instead, this method enforces
    ...
    ///
    /// If you need discriminator validation on reload, use `Account<T>` with an TrezoaAnchor
    /// #[account] type.
...
impl<'a, T: AccountSerialize + AccountDeserialize + CheckOwner + Clone> InterfaceAccount<'a, T> {
    ...
    ///
    /// This **does not** check an TrezoaAnchor discriminator. It first validates
    ...
    pub fn try_from(info: &'a AccountInfo<'a>) -> Result<Self> {
        // `InterfaceAccount` targets foreign program accounts (e.g., TPL Token
        // accounts) that do not have TrezoaAnchor discriminators. Because of that, we
        // intentionally skip the TrezoaAnchor discriminator check here and instead:
        //
    ...
    /// that both skip TrezoaAnchor discriminator checks, and `try_from` additionally
    /// enforces ownership.
    #[inline(never)]
