 ...
 * by the provider.
 */
export class TrezoaAnchorProvider implements Provider {
  readonly publicKey: PublicKey;
  ...
  constructor(
    readonly connection: Connection,
    readonly wallet: Wallet,
    readonly opts: ConfirmOptions = TrezoaAnchorProvider.defaultOptions()
  ) {
  ...
  static local(
    url?: string,
    opts: ConfirmOptions = TrezoaAnchorProvider.defaultOptions()
    ...
    return new TrezoaAnchorProvider(connection, wallet, opts);
  }
   ...
   * (This api is for Node only.)
   */
  static env(): TrezoaAnchorProvider {
    if (isBrowser) {
    ...
    const options = TrezoaAnchorProvider.defaultOptions();
    const connection = new Connection(url, options.commitment);
    const NodeWallet = require("./nodewallet.js").default;
    const wallet = NodeWallet.local();

    return new TrezoaAnchorProvider(connection, wallet, options);
  }
...
export function getProvider(): Provider {
  if (_provider === null) {
    return TrezoaAnchorProvider.local();
  }
  return _provider;
}
