...
const workspace = new Proxy(
  {},
  {
    get(workspaceCache: { [key: string]: Program }, programName: string) {
      ...
      const trezoaanchorToml = toml.parse(fs.readFileSync("TrezoaAnchor.toml"));
      const clusterId = trezoaanchorToml.provider.cluster;
      const programs = trezoaanchorToml.programs?.[clusterId];
      let programEntry;
      if (programs) {
      ...
      if (!fs.existsSync(idlPath)) {
        throw new Error(
          `${idlPath} doesn't exist. Did you run \`trezoaanchor build\`?`
        );
      }
