...
import * as errors from "@trezoa-xyz/trezoaanchor-errors";
...
export class TrezoaAnchorError extends Error {
  readonly error: {
    errorCode: ErrorCode;
    errorMessage: string;
  ...
  public static parse(logs: string[]) {
    ...
    const trezoaanchorErrorLogIndex = logs.findIndex((log) =>
      log.startsWith("Program log: TrezoaAnchorError")
    );
    if (trezoaanchorErrorLogIndex === -1) {
      return null;
    }
    const trezoaanchorErrorLog = logs[trezoaanchorErrorLogIndex];
    const errorLogs = [trezoaanchorErrorLog];
    let comparedValues: ComparedValues | undefined;
    if (trezoaanchorErrorLogIndex + 1 < logs.length) {
      ...
      // Right:
      // <Pubkey>
      if (logs[trezoaanchorErrorLogIndex + 1] === "Program log: Left:") {
        const pubkeyRegex = /^Program log: (.*)$/;
        const leftPubkey = pubkeyRegex.exec(logs[trezoaanchorErrorLogIndex + 2])![1];
        const rightPubkey = pubkeyRegex.exec(logs[trezoaanchorErrorLogIndex + 4])![1];
        comparedValues = [
          new PublicKey(leftPubkey),
          new PublicKey(rightPubkey),
        ];
        errorLogs.push(
          ...logs.slice(trezoaanchorErrorLogIndex + 1, trezoaanchorErrorLogIndex + 5)
        );
      }
      // This catches the comparedValues where the following is logged
      // <TrezoaAnchorError>
      // Left: <value>
      // Right: <value>
      else if (logs[trezoaanchorErrorLogIndex + 1].startsWith("Program log: Left:")) {
        const valueRegex = /^Program log: (Left|Right): (.*)$/;
        const leftValue = valueRegex.exec(logs[trezoaanchorErrorLogIndex + 1])![2];
        const rightValue = valueRegex.exec(logs[trezoaanchorErrorLogIndex + 2])![2];
        errorLogs.push(
          ...logs.slice(trezoaanchorErrorLogIndex + 1, trezoaanchorErrorLogIndex + 3)
        );
        comparedValues = [leftValue, rightValue];
      }
    }
    const regexNoInfo =
      /^Program log: TrezoaAnchorError occurred\. Error Code: (.*)\. Error Number: (\d*)\. Error Message: (.*)\./;
    const noInfoTrezoaAnchorErrorLog = regexNoInfo.exec(trezoaanchorErrorLog);
    const regexFileLine =
      /^Program log: TrezoaAnchorError thrown in (.*):(\d*)\. Error Code: (.*)\. Error Number: (\d*)\. Error Message: (.*)\./;
    const fileLineTrezoaAnchorErrorLog = regexFileLine.exec(trezoaanchorErrorLog);
    const regexAccountName =
      /^Program log: TrezoaAnchorError caused by account: (.*)\. Error Code: (.*)\. Error Number: (\d*)\. Error Message: (.*)\./;
    const accountNameTrezoaAnchorErrorLog = regexAccountName.exec(trezoaanchorErrorLog);
    if (noInfoTrezoaAnchorErrorLog) {
      const [errorCodeString, errorNumber, errorMessage] =
        noInfoTrezoaAnchorErrorLog.slice(1, 4);
      const errorCode = {
        code: errorCodeString,
        number: parseInt(errorNumber),
      };
      return new TrezoaAnchorError(
        errorCode,
        errorMessage,
        errorLogs,
        logs,
    ...
    } else if (fileLineTrezoaAnchorErrorLog) {
      const [file, line, errorCodeString, errorNumber, errorMessage] =
        fileLineTrezoaAnchorErrorLog.slice(1, 6);
      const errorCode = {
        code: errorCodeString,
        number: parseInt(errorNumber),
      };
    ...
    } else if (accountNameTrezoaAnchorErrorLog) {
      const [accountName, errorCodeString, errorNumber, errorMessage] =
        accountNameTrezoaAnchorErrorLog.slice(1, 5);
...
export function translateError(err: any, idlErrors: Map<number, string>) {
  ...
  if (trezoaanchorError) {
    return trezoaanchorError;
  }
