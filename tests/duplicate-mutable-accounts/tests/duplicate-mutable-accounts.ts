...
describe("duplicate-mutable-accounts", () => {
  ...
  const provider = trezoaanchor.getProvider() as trezoaanchor.TrezoaAnchorProvider;
  ...
  const user_wallet = trezoaanchor.web3.Keypair.generate();

  // Two regular system accounts to hold Counter state (must sign on init)
  const dataAccount1 = trezoaanchor.web3.Keypair.generate();
  const dataAccount2 = trezoaanchor.web3.Keypair.generate();

  it("Initialize accounts", async () => {
    // 1) Fund user_wallet so it can pay rent
    const airdropSig = await provider.connection.requestAirdrop(
      user_wallet.publicKey,
      2 * trezoaanchor.web3.LAMPORTS_PER_TRZ
    );
    await provider.connection.confirmTransaction(airdropSig);

    // 2) Create & init dataAccount1 (must sign with dataAccount1)
    await program.methods
      .initialize(new trezoaanchor.BN(100))
      .accounts({
    ...
    await program.methods
      .initialize(new trezoaanchor.BN(300))
      .accounts({
  ...
  it("Should fail with duplicate mutable accounts", async () => {
    ...
    try {
      ...
      assert.instanceOf(e, TrezoaAnchorError);
      const err = e as TrezoaAnchorError;
      assert.strictEqual(
  ...
  it("Should allow initializing multiple accounts with the same payer", async () => {
    ...
    const newAccount1 = trezoaanchor.web3.Keypair.generate();
    const newAccount2 = trezoaanchor.web3.Keypair.generate();
    ...
    await program.methods
      .initMultipleWithSamePayer(new trezoaanchor.BN(500), new trezoaanchor.BN(600))
      .accounts({
