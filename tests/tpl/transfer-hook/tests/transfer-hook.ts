...
describe("transfer hook", () => {
  ...
  const program = trezoaanchor.workspace.TransferHook as Program<TransferHook>;
  ...
  it("can create an `InitializeExtraAccountMetaList` instruction with the proper discriminator", async () => {
    ...
    assert.equal(
      ix.data.subarray(0, 8).compare(
        Buffer.from([43, 34, 13, 49, 167, 88, 235, 235]) // TPL discriminator for `InitializeExtraAccountMetaList` from interface
      ),
      0
    );
    const { name, data } = new trezoaanchor.BorshInstructionCoder(program.idl).decode(
      ix.data,
      "hex"
    );
  ...
  it("can create an `Execute` instruction with the proper discriminator", async () => {
    const ix = await program.methods
      .execute(new trezoaanchor.BN(transferAmount))
      .accounts({
    ...
    assert.equal(
      ix.data.subarray(0, 8).compare(
        Buffer.from([105, 37, 101, 197, 75, 251, 102, 26]) // TPL discriminator for `Execute` from interface
      ),
      0
    );
    const { name, data } = new trezoaanchor.BorshInstructionCoder(program.idl).decode(
    ...
    assert.isTrue(trezoaanchor.BN.isBN(data.amount));
    assert.isTrue(data.amount.eq(new trezoaanchor.BN(transferAmount)));
  });
