...
describe("program", () => {
  ...
  const TEST_PROGRAM_IDS = [
    TPL_TOKEN_PROGRAM_ID,
    new trezoaanchor.web3.PublicKey("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb"),
  ];
  ...
  const program = trezoaanchor.workspace.TokenProxy;

  TOKEN_PROGRAMS.forEach((tokenProgram) => {
    const name = tokenProgram.programId.equals(TPL_TOKEN_PROGRAM_ID)
      ? "token"
      : "token-2022";
    describe(name, () => {
      let mint = null;
      ...
      it("Creates a token account", async () => {
        ...
        const [tokenAccount] = trezoaanchor.web3.PublicKey.findProgramAddressSync(
          [
            authority.toBytes(),
            newMint.toBytes(),
        ...
        assert.isTrue(account.amount.eq(new trezoaanchor.BN(0)));
      });

      it("Creates an associated token account", async () => {
        const newMint = await createMint(tokenProgram);
        const authority = provider.wallet.publicKey;
        const associatedTokenProgram = new trezoaanchor.web3.PublicKey(
      ...
      it("Creates a mint", async () => {
        const authority = provider.wallet.publicKey;
        const [newMint] = trezoaanchor.web3.PublicKey.findProgramAddressSync(
          [
            authority.toBytes(),
            Buffer.from(name),
      ...
      it("Mints a token", async () => {
        await program.rpc.proxyMintTo(new trezoaanchor.BN(1000), {
          accounts: {
        ...
        assert.isTrue(fromAccount.amount.eq(new trezoaanchor.BN(1000)));
      });
      ...
      it("Transfers a token", async () => {
        ...
        const transferAmount = new trezoaanchor.BN(400);
      ...
      it("Transfers a token with optional accounts", async () => {
        ...
        const transferAmount = new trezoaanchor.BN(10);
      ...
      it("Does not transfer a token without optional accounts", async () => {
        ...
        const optionalTransferIx = await program.methods
          .proxyOptionalTransfer(new trezoaanchor.BN(10))
          .accounts({
            authority: provider.wallet.publicKey,
            to,
            from,
          ...
          .instruction();
        const tx = new trezoaanchor.web3.Transaction().add(optionalTransferIx);
      ...
      it("Burns a token", async () => {
        ...
        const burnAmount = new trezoaanchor.BN(300);
      ...
      it("Set new mint authority", async () => {
        const newMintAuthority = trezoaanchor.web3.Keypair.generate();
...
// TPL token client boilerplate for test initialization. Everything below here is
