...
describe("wrapper", () => {
  ...
  const program = trezoaanchor.workspace.TokenWrapper as Program<TokenWrapper>;
    ...
    describe(name, () => {
      it("Initialise wrapper", async () => {
        ...
        const [_wrapperAuthority, _] = PublicKey.findProgramAddressSync(
          [
            Buffer.from(trezoaanchor.utils.bytes.utf8.encode("wrapr")),
        ...
        const [_depositTokenVault, __] = PublicKey.findProgramAddressSync(
          [
            Buffer.from(trezoaanchor.utils.bytes.utf8.encode("vault")),
            depositMint.publicKey.toBuffer(),
            wrappedMintKP.publicKey.toBuffer(),
          ],
          program.programId
