...
describe("Test CLI IDL commands", () => {
  ...
  const programOne = trezoaanchor.workspace.IdlCommandsOne as Program<IdlCommandsOne>;
  const programTwo = trezoaanchor.workspace.IdlCommandsTwo as Program<IdlCommandsTwo>;

  it("Can initialize IDL account", async () => {
    execSync(
      `trezoaanchor idl init --filepath target/idl/idl_commands_one.json ${programOne.programId}`,
      { stdio: "inherit" }
    );
  });

  it("Can fetch an IDL using the TypeScript client", async () => {
    const idl = await trezoaanchor.Program.fetchIdl(programOne.programId, provider);
    assert.deepEqual(idl, programOne.rawIdl);
  });

  it("Can fetch an IDL via the CLI", async () => {
    const idl = execSync(`trezoaanchor idl fetch ${programOne.programId}`).toString();
  ...
  it("Can write a new IDL using the upgrade command", async () => {
    ...
    execSync(
      `trezoaanchor idl upgrade --filepath target/idl/idl_commands_two.json ${programOne.programId}`,
      { stdio: "inherit" }
    );
    const idl = await trezoaanchor.Program.fetchIdl(programOne.programId, provider);
  ...
  it("Can write a new IDL using write-buffer and set-buffer", async () => {
    ...
    let buffer = execSync(
      `trezoaanchor idl write-buffer --filepath target/idl/idl_commands_one.json ${programOne.programId}`
    ).toString();
    buffer = buffer.replace("Idl buffer created: ", "").trim();
    execSync(
      `trezoaanchor idl set-buffer --buffer ${buffer} ${programOne.programId}`,
      { stdio: "inherit" }
    );
    const idl = await trezoaanchor.Program.fetchIdl(programOne.programId, provider);
    assert.deepEqual(idl, programOne.rawIdl);
  ...
  it("Can fetch an IDL authority via the CLI", async () => {
    const authority = execSync(`trezoaanchor idl authority ${programOne.programId}`)
      .toString()
      .trim();
  ...
  it("Can close IDL account", async () => {
    execSync(`trezoaanchor idl close ${programOne.programId}`, { stdio: "inherit" });
    const idl = await trezoaanchor.Program.fetchIdl(programOne.programId, provider);
    assert.isNull(idl);
  });
  ...
  it("Can initialize super massive IDL account", async () => {
    execSync(
      `trezoaanchor idl init --filepath testLargeIdl.json ${programOne.programId}`,
      { stdio: "inherit" }
    );
    const idlActual = await trezoaanchor.Program.fetchIdl(
